...
import Animated, { Extrapolation, interpolate, interpolateColor, ReduceMotion, runOnJS, useAnimatedStyle, useDerivedValue, useSharedValue, withClamp, withDecay, withSpring } from "react-native-reanimated";
import GestureHandler from "./GestureHandler";
import { Gesture, GestureDetector, GestureHandlerRootView } from "react-native-gesture-handler";
import { snapPoint, useSpring } from "react-native-redash";
...
const Picker = ({ values, defaultValue }: PickerProps) => {
  ...
  .onEnd((event) => {
    ...
    translateY.value = withDecay({
      ...
      deceleration: 0.998,
      clamp: [-ITEM_HEIGHT * (hours.length - VISIBLE_ITEMS), 0],
      velocityFactor: 1,
      ...
      // callback
        ...
        else if (translateY.value <= -ITEM_HEIGHT * (hours.length - VISIBLE_ITEMS)) {
          // Do Nothing
        }
        else if (Math.abs(translateY.value % 32) < 16) {
          console.log(`scrolling to ${3}`)
          scrollTo((Math.floor(translateY.value / 32) + 1) * ITEM_HEIGHT)
        }
        else if (Math.abs(translateY.value % 32) >= 16) {
          console.log(`scrolling to ${4}`)
          scrollTo((Math.floor(translateY.value / 32)) * ITEM_HEIGHT)
        }
    })
  })
        ...
        hours.map((v, i) => {
          ...
          return (
              <Animated.View
                ...
                >
                <Animated.Text
                  style={[styles.label, colorAnimate]}>
                    {v}
                  </Animated.Text>
              </Animated.View>
          )
