...
interface PickerProps {
  ...
  textStyle?: { fontFamily?: string, fontSize?: number, primaryColor?: string, secondaryColor?: string };
  moreTextStyles?: {};
  moreViewStyles?: {};
...
const Picker = ({ values, legendState, moreTextStyles, defaultValue = 0, VISIBLE_ITEMS = 5, ITEM_HEIGHT = 32, width = 100, textStyle = { fontFamily: "SFProText-Semibold", fontSize: 24, primaryColor: 'black', secondaryColor: 'gray' }, viewStyle = { perspective: 600, RADIUS_REL_Factor: 0.5}, enableSelectBox = true }: PickerProps) => {
  ...
  .onEnd((event) => {
    translateY.value = withDecay({
      ...
      runOnJS(update)(Math.round(Math.abs(translateY.value / ITEM_HEIGHT - 2)));
    })
  })
  ...
  const maskElement = (
    <View style={[styles.container]}>
      ...
      <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, justifyContent: 'center', alignItems: 'center'}}>
        <View style={{height: ITEM_HEIGHT, width: width, backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 16,}} />
      </View>
      }
